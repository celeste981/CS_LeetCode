...
//    }
    ...
    public List<List<Integer>> combinationSum ( int[] candidates , int target ) {
        List<List<Integer>> ans = new ArrayList<>();
        Deque<Integer> path = new ArrayDeque<>();
        dfs( ans , path , 0 , candidates , target );
        return ans;
    }

    public void dfs ( List<List<Integer>> ans , Deque<Integer> path , int begin , int[] candidates , int target ) {
        if ( target == 0 ) {
            ans.add( new ArrayList<>(path) );
        }
        for ( int i = begin ; i < candidates.length ; i++ ) {
            ...
            dfs( ans, path, i , candidates ,target - candidates[i]);
            path.removeLast();
        }
    }
